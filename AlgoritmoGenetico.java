    ...
    public Individuo resolver(){
        ...
        for(int i = 0; i < Globals.tamPopulacao; i++){     
            ...
            for(int j = 0; j < Globals.pontosIniciais.size(); j++){
                ...
                do{
                    x = gen.nextDouble() * (Globals.maiorx + Globals.raio);
                    y = gen.nextDouble() * (Globals.maiory + Globals.raio); 
                    np = new Ponto(x, y);
                }while(Globals.calcDistancia(p, np) > Globals.raio);

                pontos.add(np);
            }
//            for(Ponto p : Globals.pontosIniciais){
//                
//                double x = 0;
//                double y = 0;
//                Ponto np = null;
...
//                
//            }
            Individuo indi = new Individuo(pontos);
            indi.avaliacao();
            indi.distancia();
    ...
    public ArrayList<Ponto> HillClimb(){
    ...
    public boolean verificaVisitados(ArrayList<Ponto> solucao){
        for(Ponto p1 : solucao){
            for(Ponto p2 : Globals.pontosIniciais){
                if(Globals.calcDistancia(p1,p2) > Globals.raio){
                    return false;
                }
            }
